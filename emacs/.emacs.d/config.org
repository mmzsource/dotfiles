...
Since I'm using homebrew I can simply run ~brew update~ and then ~brew reinstall
emacs --cask --no-quarantine~. (See
https://github.com/Homebrew/homebrew-cask/issues/87415) After upgrade, it's wise
to debug the emacs setup, see [[Debugging][Debugging]]. If your setup is ruined beyond repair,
you can link your environment back to the old brew installation (using brew).
That way, you can wait for a more stable release while continuing to work in
your old setup.

My experience going from emacs 25.3 to 26.1, from 26.1 to 26.3 from 26.3 to 27.1
and from 27.1 to 27.2. was that nothing was broken and nothing needed to be fixed.
I could simple continue my work.
...
Gnuplot mode on top of gnuplot (brew install gnuplot) helps plotting org-tables
among others. https://github.com/emacsorphanage/gnuplot

#+BEGIN_SRC emacs-lisp
(message "%s" "MMz: Start loading gnuplot")
...
Packages are stored in ~.emacs.d/elpa/~ with their version as part of the folder
name. Packages aren't upgraded automatically (by choice; there are packages that
auto update, but I want to decide when and how to upgrade). To upgrade:

- Backup your ~/.emacs.d/elpa/~ folder so you always can go back to a working
  situation
...
If emacs crashes, a ~.emacs.desktop.lock~ file will remain in the ~.emacs.d~ 
directory. This will trigger a startup warning *every single time*:

"desktop file appears to be in use by PID ..."

... until the lock file is simply removed.
